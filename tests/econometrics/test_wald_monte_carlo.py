...
def test_wald_size_on_circle():
    ...
    for _ in range(n_reps):
        ...
        restriction = MomentRestriction(
            gi_jax=gi_jax, data=jnp.array(data), manifold=manifold, backend="jax"
        )
    ...
    assert (
        0.0 <= rejection_rate <= 0.15
    ), f"Rejection rate {rejection_rate} is too far from nominal {alpha_level}"
