...
def test_psd_wald_power_advantage():
    ...
    # A_true = v_true @ v_true.T is the rank-1 covariance under H1 (kept as
    # documentation for the DGP comment below; samples are drawn from
    # ``z @ v_true.T`` plus noise, which has the same rank-1 structure).
    ...
    for _ in range(n_reps):
        ...
        res_man = GMM(
            MomentRestriction(
                gi_jax=gi_jax, data=data_jax, manifold=manifold_man, backend="jax"
            ),
