...
#+begin_src python
  ...
  angles = np.linspace(0, 0.20, 11) # Finer grid for small angles
  n_reps_curve = 200
...
#+CAPTION: Power Curve: Manifold vs Euclidean Wald Test
[[file:power_curve.png]]

The Euclidean estimator treats the data as living in \(\mathbb{R}^2\). Although the sample mean is shrunk towards the origin (bias), the Wald statistic is studentized (divided by variance). In this specific case (testing a linear restriction on the mean direction), the shrinkage factor cancels out, leading to very similar power curves for both the Manifold and Euclidean tests. This demonstrates that for simple hypotheses, standard Euclidean inference can be robust, but Manifold GMM provides a rigorous framework for more complex geometries where such cancellation might not occur.