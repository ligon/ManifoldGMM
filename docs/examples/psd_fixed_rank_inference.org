...
\]
where:
- $z_i \sim N(0, 1)$ is a scalar latent factor
- $v \in \mathbb{R}^p$ is the factor loading vector
- $\varepsilon_i \sim N(0, \sigma^2 I_p)$ is idiosyncratic noise

The covariance of $x_i$ decomposes as:
\[
\text{Cov}(x) = v v^\top + \sigma^2 I = \underbrace{A}_{\text{rank-1 signal}} + \underbrace{\sigma^2 I}_{\text{noise}}
\]

** What Does Each Estimator Target?

Both estimators use the moment condition $E[x_i x_i^\top - \Theta] = 0$,
which identifies $\Theta = E[x_i x_i^\top] = \text{Cov}(x)$.
...
However, the full covariance has $\text{Cov}(x)_{22} = 0 + \sigma^2 = \sigma^2 > 0$.
...
We now vary the effect size $v_1$ from 0 (null true) to 0.3 and compute
...
#+begin_src python :results output
  ...
  # Run simulation (reduce n_reps for faster execution)
...
#+begin_src python :results file :file psd_power_curve.png
...
- But the Euclidean estimator targets $\text{Cov}(x)_{22} = \sigma^2 \neq 0$.
...
1. *Structural knowledge:* You know the parameter has low-rank or other
   geometric structure (e.g., covariance matrices in factor models,
   rotation matrices, correlation matrices).

2. *Robustness to noise:* The constraint acts as regularization,
   preventing the estimator from fitting noise.

3. *Efficiency:* Fewer parameters means lower variance for the
