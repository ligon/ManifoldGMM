...
This example compares inference for a positive semi-definite matrix of known rank under two approaches:
- Unconstrained (Euclidean) :: Estimate all unique elements of a symmetric matrix.
- Constrained (Manifold) :: Estimate a matrix constrained to have fixed low rank.

We show that the manifold approach implicitly actually extracts linear "factors", as in factor analysis, separating low-rank signal from idiosyncratic noise---a property the
...
The manifold constraints in GMM are not just about efficiency---they can fundamentally change what the estimator targets.  For estimation of covariance matrices, the rank constraint extracts signal from noise, enabling valid inference on the factor structure even in the presence of idiosyncratic variation.
