...
The criterion-based approach above relies on the asymptotic \(\chi^2\)
distribution to set critical values.  On curved manifolds with moderate
samples, this approximation can be inadequate.  A complementary approach uses
the *moment wild bootstrap* to obtain data-driven critical values that
automatically adapt to the manifold geometry.

The bootstrap procedure resamples moment *errors* (not observation pairs),
re-estimates on \(\mathcal{M}\), and computes geodesic Mahalanobis distances
from \(\hat\theta\).  The \((1 - \alpha)\) quantile of the bootstrap distance
distribution replaces the \(\chi^2\) critical value.

- Implementation :: The =MomentWildBootstrap= class in
  =econometrics/bootstrap.py= provides the full pipeline: task generation,
  sequential or parallel execution, distance computation, and membership
  tests.  See [[file:wild_bootstrap.org][wild_bootstrap.org]] for the
  detailed design.
- Relationship :: The bootstrap gives calibrated coverage; the criterion-based
  approach gives boundary tracing for visualization.  They complement rather
...
- Bootstrap integration :: *Resolved.* The moment wild bootstrap is implemented as a standalone class (=MomentWildBootstrap=) that complements the criterion-based regions.  See [[file:wild_bootstrap.org][wild_bootstrap.org]].
